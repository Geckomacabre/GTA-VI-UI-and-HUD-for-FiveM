--[[ ---------------------------------------------------------------------------
     ...
     ---------------------------------------------------------------------------
     qb-input draws its own NUI page. The theme patch in ox_lib only reaches
     ox_lib's page -- SendNUIMessage delivers to the CALLING resource's page and
     nothing else -- so these dialogs need their own copy of the hook, living
     inside qb-input. That is the whole reason this file exists rather than being folded
     ...
     html/vice-theme.css falls back to qb-input's stock appearance.

     NOTE: a qb-input update deletes this file, drops its line from
     ...
     dialogs go back to the stock Material look on their own.
     ------------------------------------------------------------------------ ]]
...
--- ox_lib gets (success/error/inform) have no counterpart here -- a form field
--- is not a notification -- and sending them would suggest vice-theme.css does
--- something with them.
local function push()
...
CreateThread(function()
    ...
    -- cost nothing and save a "why is this dialog unstyled until I retheme".
    for _ = 1, 10 do
        Wait(500)
