...
--[[
		Exports for other resources (ox_inventory's ITEMS wheel, currently) to
		toggle Mask/Hat/Glasses and read their current on/off state without
		needing to know about drawable/prop ids themselves.
]]--

exports('ToggleClothingWheelSlot', function(role)
	if role == 'mask' then return ToggleClothing('Mask')
	elseif role == 'hat' then return ToggleProps('Hat')
	elseif role == 'eyewear' then return ToggleProps('Glasses')
	end
	return false
end)

-- Read directly off the ped rather than LastEquipped: LastEquipped only
-- tracks what this resource itself last put on/took off, so it goes stale
-- the moment a mask/hat/glasses changes for any other reason (clothing
-- store, character reload, etc). The ped's actual drawable/prop is always
-- current.
exports('GetClothingWheelState', function()
	local ped = PlayerPedId()
	return {
		mask = GetPedDrawableVariation(ped, Drawables.Mask.Drawable) ~= 0,
		hat = GetPedPropIndex(ped, Props.Hat.Prop) ~= -1,
		eyewear = GetPedPropIndex(ped, Props.Glasses.Prop) ~= -1,
	}
end)
