...
lib.callback.register('ox_inventory:swapItems', function(source, data)
	...
	if toInventory and (data.toType == 'newdrop' or fromInventory ~= toInventory or data.fromSlot ~= data.toSlot) then
		...
		if fromData then
            ...
            -- Equipment frames only accept their declared categories. The UI
            -- greys out illegal drops via canPlaceInSlot(), but this is the
            -- authoritative check — a tampered NUI gains nothing.
            if toInventory.player and not Equipment.canHold(data.toSlot, Items(fromData.name)) then
                return false, 'cannot_perform'
            end

            -- A swap also pushes the destination item back into fromSlot, so
            -- that direction has to clear the gate too.
            if toData and fromInventory.player and not Equipment.canHold(data.fromSlot, Items(toData.name)) then
                return false, 'cannot_perform'
            end
...
function Inventory.GetEmptySlot(inv)
	...
	-- Reserved equipment frames are skipped so ordinary pickups never land in
	-- them; only player inventories have frames to begin with.
	local gated = inventory.player and Equipment.gatedSlots

	for i = 1, inventory.slots do
		if not items[i] and not (gated and gated[i]) then
			return i
		end
	end
end
...
function Inventory.GetSlotForItem(inv, itemName, metadata)
	...
	local items = inventory.items
	local emptySlot
	-- Stacking onto an item already sitting in a frame stays allowed; only the
	-- empty-slot fallback skips them, so pickups don't auto-fill equipment.
	local gated = inventory.player and Equipment.gatedSlots
	...
	for i = 1, inventory.slots do
		...
		if item.stack and slotData and slotData.name == item.name and table.matches(slotData.metadata, metadata) then
			return i
		elseif not item.stack and not slotData and not emptySlot and not (gated and gated[i]) then
			emptySlot = i
		end
	end
...
exports('itemcreatorRebuildStashes', function(list)
	for name in pairs(RegisteredStashes) do
		RegisteredStashes[name] = nil
	end

	for _, stash in pairs(list or {}) do
		if type(stash) == 'table' and type(stash.name) == 'string' then
			RegisteredStashes[stash.name] = {
				...
				maxWeight = stash.weight,
				groups = stash.groups or stash.jobs,
				coords = shared.target and stash.target and stash.target.loc or stash.coords,
				distance = stash.distance or 10
			}
		end
	end
