...
if IsDuplicityVersion() then
    ...
    client = {
        ...
        -- The fallback matters: it is what the keybind defaults are built from,
        -- and a wrong one is invisible until someone notices the wrong key
        -- opens the wrong thing. Decode defensively -- json.decode('') is not
        -- guaranteed to return nil rather than raise -- and default to what
        -- ox.cfg actually sets, so an unset/unreplicated convar lands in the
        -- same place instead of silently reverting to upstream's F2/K/TAB.
        keys = (function()
            local ok, decoded = pcall(json.decode, GetConvar('inventory:keys', ''))

            if ok and type(decoded) == 'table' and decoded[1] then return decoded end
        ...
        -- Dumpsters on this server belong to um_hobos' scavenging, which rolls
        -- its own loot tables and grants hobo XP. Leaving the stock dumpster
        -- loot on top of it put three separate "search" options on every bin.
        -- Set inventory:dumpsters 1 to hand them back to ox_inventory.
        dumpsters = GetConvarInt('inventory:dumpsters', 0) == 1,
