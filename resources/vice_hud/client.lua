...
-- Redefines the placeholder palette slot Config.Nav.waypointColour points
-- at to the exact RGB this HUD wants, once, at resource start — see the
-- comment on waypointColour/waypointColourRGB in config.lua for why a
-- placeholder index rather than HUD_COLOUR_PINK itself. A plain top-level
-- call rather than an event handler: this file already only runs once per
-- resource start, so there's nothing an onClientResourceStart handler would
-- add here. Restored back to white on resource stop — see the
-- onResourceStop handler near the end of this file.
if Config.Nav.waypointColour and Config.Nav.waypointColourRGB then
    local rgb = Config.Nav.waypointColourRGB
    ReplaceHudColourWithRgba(Config.Nav.waypointColour, rgb[1], rgb[2], rgb[3], 255)
end
...
local function getWaypointCoords()
    ...
    if blip and blip ~= 0 and DoesBlipExist(blip) then
        local idx = Config.Nav.waypointColour
        if idx and blip ~= colouredWaypointBlip then
            -- Both the cross and the route line take a real HUD_COLOUR_*
            -- palette index -- there is no custom-RGB passthrough for either
            -- of them despite community claims otherwise. Confirmed wrong
...
AddEventHandler('onResourceStop', function(res)
    ...
    -- the client session, not just while this resource runs. Put back the
    -- stock white a placeholder index ships as (see where this was set,
    -- near the top of the file) so nothing else that might reach for a
    -- placeholder slot inherits vice_hud's pink after this resource stops.
    if Config.Nav.waypointColour and Config.Nav.waypointColourRGB then
        ReplaceHudColourWithRgba(Config.Nav.waypointColour, 255, 255, 255, 255)
    end
end)
